...
public class BaseUtils
{
	...
	public static BufferedImage getLocalImage(String name)
	{
		try
		{
			...
			BufferedImage img = ImageIO.read(BaseUtils.class.getResource("/src/net/launcher/theme/" + name + ".png"));
			imgs.put(name, img);
	...
	public static Font getFont(String name, float size)
	{
		try
		{
			...
			Font font = Font.createFont(Font.TRUETYPE_FONT, BaseUtils.class.getResourceAsStream("/src/net/launcher/theme/" + name + ".ttf"));
			fonts.put(name, font);
	...
	public static String genServerStatus(String[] args)
	{
		if (servtype == 1)
		{
			...
			{
				if(args[4].equals(args[5])) return "  Сервер переполнен";
				return "Онлайн " + args[4] + " из " + args[5];
			}
		}
		else
		if (servtype == 2)
		...
		{
			if(args[1].equals(args[2])) return "  Сервер переполнен";
			return "На сервере " + args[1] + " из " + args[2];
		}
		}
	...
	public static void patchDir( URLClassLoader cl ) {
		...
		try {
			Class< ? > c = cl.loadClass("src.net.minecraft.client.Minecraft");

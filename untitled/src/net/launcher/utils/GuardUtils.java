package src.net.launcher.utils;
...
import src.net.launcher.components.Frame;
import src.net.launcher.components.Game;
...
public class GuardUtils
{
	...
	public static void checkMods(String answer, boolean action)
	{
		if(!Frame.main.offline.isSelected())
		{
			...
			if(!EncodingUtils.xorencode(EncodingUtils.inttostr(answer.split("<br>")[0].split("<:>")[3]), Settings.protectionKey).equals(GuardUtils.getMD5(binfolder + src.net.launcher.utils.ThreadUtils.l))) ret = true;
			if(!EncodingUtils.xorencode(EncodingUtils.inttostr(answer.split("<br>")[0].split("<:>")[4]), Settings.protectionKey).equals(GuardUtils.getMD5(binfolder + src.net.launcher.utils.ThreadUtils.f))) ret = true;
			if(!EncodingUtils.xorencode(EncodingUtils.inttostr(answer.split("<br>")[0].split("<:>")[5]), Settings.protectionKey).equals(GuardUtils.getMD5(binfolder + src.net.launcher.utils.ThreadUtils.e))) ret = true;
			if(!EncodingUtils.xorencode(EncodingUtils.inttostr(answer.split("<br>")[0].split("<:>")[2]), Settings.protectionKey).equals(GuardUtils.getMD5(binfolder + src.net.launcher.utils.ThreadUtils.m))) ret = true;
